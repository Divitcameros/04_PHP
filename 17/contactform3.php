...
// 設定ファイルの読み込み
require_once(dirname(__FILE__) . '/mailEnvAdmin.php');
...
if (!isset($error['name']) && !isset($error['email']) && !isset($error['detail'])) {
  ...
  // ユーザー用
  $mailerUser  = new Swift_Mailer($transport);
  

  // 管理者用メール処理
  // メールメッセージのインスタンスを生成
  $msgAdmin = new Swift_Message(MAIL_TITLE_ADMIN);
  // メールの差出人を指定
  $msgAdmin->setFrom(MAIL_FROM);
  // メールの送信先を指定
  $msgAdmin->setTo(MAIL_TO_ADMIN);
  // メール本文にヒアドキュメントの内容を指定(HTMLタグを使用する)
  $msgAdmin->setBody(setMailBody($msgAdmin, $messageArr, 'admin'), 'text/html');
  

  // ユーザー用メール処理
  // メールメッセージのインスタンスを生成
  $msgUser = new Swift_Message(MAIL_TITLE_USER);
  // メールの差出人を指定
  $msgUser->setFrom(MAIL_FROM);
  // メールの送信先を指定
  $msgUser->setTo(setUserMail($email, $name));
  // メール本文にヒアドキュメントの内容を指定(HTMLタグを使用する)
  $msgUser->setBody(setMailBody($msgUser, $messageArr, 'user'), 'text/html');
...
<body>
  <?php if ($resultAdmin == 1 && $resultUser == 1) : ?>
    <h1>お問い合わせフォーム</h1>
